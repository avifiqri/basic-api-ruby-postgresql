...
class ExamplesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create example" do
    assert_difference('Example.count') do
      post examples_url, params: { example: { id: @example.id, name: @example.name } }, as: :json
    end
  ...
  test "should update example" do
    patch example_url(@example), params: { example: { id: @example.id, name: @example.name } }, as: :json
    assert_response 200
  end
